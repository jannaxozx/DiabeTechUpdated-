...
class _AdminReportsScreenState extends State<AdminReportsScreen>
  ...
  Future<void> _fUsers() async {
    final s = await FirebaseFirestore.instance
        .collection('users').where('role', isEqualTo: 'user').get();
    ...
    if (mounted) setState(() {
      _totalUsers = s.docs.length; _mildUsers = mild;
      _severeUsers = severe; _otherUsers = other; _activityDist = act;
    });
  }
  ...
  Future<void> _fFoods() async {
    ...
    for (final doc in s.docs) {
      ...
      if (dt == 'Mild') mildC++;
      else if (dt == 'Severe') sevC++;
    }
    if (mounted) setState(() {
      _totalFoods = s.docs.length; _doFoods = doC; _dontFoods = dontC;
      _mildFoods = mildC; _severeFoods = sevC;
    });
  }

  Future<void> _fLogs() async {
    final s = await FirebaseFirestore.instance
        .collectionGroup('foodLogs').limit(500).get();
    ...
    for (final doc in s.docs) {
      ...
      tCal  += _dbl(n['calories']); tCarbs += _dbl(n['carbs']);
      tProt += _dbl(n['protein']);  tFat   += _dbl(n['fat']);
    ...
    final top5 = Map.fromEntries(
        (food.entries.toList()..sort((a, b) => b.value.compareTo(a.value)))
            .take(5));
    ...
    if (mounted) setState(() {
      ...
      _mealTypeDist = meal; _topFoodsLogged = top5; _logsPerDay = day;
    });
  }

  Future<void> _fScans() async {
    final s = await FirebaseFirestore.instance
        .collectionGroup('scanned_foods').limit(500).get();
    ...
    final top5 = Map.fromEntries(
        (food.entries.toList()..sort((a, b) => b.value.compareTo(a.value)))
            .take(5));
    if (mounted) setState(() {
      _totalScans  = s.docs.length; _safeScans = safe;
      _avoidScans  = avoid;         _unknownScans = unk;
      _topFoodsScanned = top5;      _scansPerDay = day;
    });
  }
      ...
      child: SafeArea(
        ...
        child: Column(children: [
                ...
                child: Column(
                  ...
                  Text('Analytics & Reports',
                      style: TextStyle(
                          ...
                          letterSpacing: 0.1)),
                  Text('DiabeTech Admin Dashboard',
                      style: TextStyle(
          ...
          TabBar(
            ...
            tabs: const [
              Tab(icon: Icon(Icons.people_alt_rounded,      size: 17), text: 'Users'),
              Tab(icon: Icon(Icons.restaurant_menu_rounded, size: 17), text: 'Foods'),
              Tab(icon: Icon(Icons.edit_note_rounded,       size: 17), text: 'Logs'),
              Tab(icon: Icon(Icons.qr_code_scanner_rounded, size: 17), text: 'Scans'),
            ],
          ),
        ]),
      ),
  ...
  Widget _usersTab() => SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start,
          ...
          // KPI row
          Row(children: [
            Expanded(child: _kpi('Total Users', '$_totalUsers',
                Icons.people_alt_rounded, _blue, _bluePal)),
            const SizedBox(width: 10),
            Expanded(child: _kpi('Mild', '$_mildUsers',
                Icons.health_and_safety_rounded, _green, _greenPal)),
            const SizedBox(width: 10),
            Expanded(child: _kpi('Severe', '$_severeUsers',
                Icons.warning_amber_rounded, _red, _redPal)),
          ]),
          const SizedBox(height: 20),
          ...
          const SizedBox(height: 10),
          _card(_activityDist.isEmpty
              ? _empty()
              : Column(children: [
                  ..._activityDist.entries.map((e) => Padding(
                    padding: const EdgeInsets.only(bottom: 16),
                    child: _hBar(
                        e.key, e.value, _totalUsers, _actColor(e.key)),
                  )).toList(),
                ])),
  ...
  Widget _foodsTab() => SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Row(children: [
            Expanded(child: _kpi('Total', '$_totalFoods',
                Icons.restaurant_menu_rounded, _teal, _tealPal)),
            const SizedBox(width: 10),
            Expanded(child: _kpi('Safe', '$_doFoods',
                Icons.check_circle_rounded, _green, _greenPal)),
            const SizedBox(width: 10),
            Expanded(child: _kpi('Avoid', '$_dontFoods',
                Icons.cancel_rounded, _red, _redPal)),
          ]),
          const SizedBox(height: 20),
          ...
          const SizedBox(height: 10),
          _card(Column(children: [
            _sRow(Icons.restaurant_menu_rounded, _teal,  'Total food items',        '$_totalFoods'),
            _divider(),
            _sRow(Icons.check_circle_rounded,    _green, 'Recommended foods',       '$_doFoods'),
            _divider(),
            _sRow(Icons.cancel_rounded,          _red,   'Foods to avoid',          '$_dontFoods'),
            _divider(),
            _sRow(Icons.health_and_safety_rounded, _green,'Mild coverage',          '$_mildFoods items'),
            _divider(),
            _sRow(Icons.warning_amber_rounded,   _red,   'Severe coverage',         '$_severeFoods items'),
          ])),
          const SizedBox(height: 8),
        ]),
      );
  ...
  Widget _logsTab() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        ...
        // Hero total
        _totalBanner(_totalLogs, 'Total Meal Logs',
            Icons.edit_note_rounded, _green, _greenPal),
        const SizedBox(height: 20),

        _sectionLabel('Average Nutrition Per Log'),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _nutCard('Calories',
              _avgCal.toStringAsFixed(0), 'kcal',
              Icons.local_fire_department_rounded, _amber, _amberPal)),
          const SizedBox(width: 10),
          Expanded(child: _nutCard('Carbs',
              _avgCarbs.toStringAsFixed(1), 'g',
              Icons.grain_rounded, _blue, _bluePal)),
        ]),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _nutCard('Protein',
              _avgProtein.toStringAsFixed(1), 'g',
              Icons.fitness_center_rounded, _green, _greenPal)),
          const SizedBox(width: 10),
          Expanded(child: _nutCard('Fat',
              _avgFat.toStringAsFixed(1), 'g',
              Icons.opacity_rounded, _red, _redPal)),
        ]),
        const SizedBox(height: 20),

        _sectionLabel('Meal Type Breakdown'),
        const SizedBox(height: 10),
        _card(_mealTypeDist.isEmpty
            ? _empty()
            : Column(children: [
                ..._mealTypeDist.entries.map((e) => Padding(
                  padding: const EdgeInsets.only(bottom: 16),
                  child: _hBar(
                      e.key, e.value, _totalLogs, _mealColor(e.key)),
                )).toList(),
              ])),
        const SizedBox(height: 20),
        ...
        _card(days.isEmpty
            ? _empty()
            : _barChart(days, _logsPerDay, _green)),
        const SizedBox(height: 8),
      ]),
    );
  }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        ...
        Row(children: [
          ...
          Expanded(child: _kpi('Safe', '$_safeScans',
              Icons.check_circle_rounded, _green, _greenPal)),
          const SizedBox(width: 10),
          Expanded(child: _kpi('Avoided', '$_avoidScans',
              Icons.cancel_rounded, _red, _redPal)),
        ]),
        const SizedBox(height: 20),
  ...
  Widget _totalBanner(int total, String label, IconData icon,
      Color color, Color palColor) =>
      Container(
        padding: const EdgeInsets.all(18),
        decoration: BoxDecoration(
  ...
  /// Nutrition avg card
  Widget _nutCard(String label, String value, String unit,
      IconData icon, Color color, Color palColor) =>
      Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: _white,
        ...
        child: Row(children: [
          Container(
            ...
            decoration: BoxDecoration(
                color: palColor, borderRadius: BorderRadius.circular(10)),
            child: Icon(icon, color: color, size: 18),
          ),
          const SizedBox(width: 10),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _barChart(
      List<String> days, Map<String, int> data, Color color) {
    final maxVal =
        data.values.fold(0, (a, b) => a > b ? a : b).toDouble();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: barH > 0
                            ? (isMax ? color : color.withOpacity(0.35))
                            : _grey4,
  ...
  Color _actColor(String level) {
    switch (level) {
      case 'Sedentary':   return _grey3;
      case 'Light':       return _green;
      case 'Very Active': return _red;
      default:            return _grey2;
    }
  }
