...
class _DashboardState extends State<Dashboard> {
  ...
  _DiabetesNutrientGoals _resolveGoals(String type, Map<String, dynamic> userData) {
    ...
    if (h != null && w != null && h > 0 && w > 0) {
      ...
      debugPrint('Goals → BMR+TDEE | Cal:${g.calories} Carbs:${g.carbs} '
  ...
  Future<void> _fetchDashboardData() async {
    ...
    try {
      ...
      final foodLogsSnap = await _firestore
          .collection('users').doc(user!.uid).collection('foodLogs').get();
      ...
      for (var doc in foodLogsSnap.docs) {
        ...
        if (ts != null && (data['imagePath'] != null || data['scannedFood'] == true)) {
    ...
    try {
      final scannedFoodsSnap = await _firestore
          .collection('users').doc(user!.uid).collection('scanned_foods').get();
      ...
      if (mounted) setState(() => recentScans = fetchedScans.take(10).toList());
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [
                        Expanded(child: Text('Swipe to see all  •  Log meals to update',
                            style: TextStyle(fontSize: 10, color: _grey3))),
                      ...
                      SizedBox(
                        ...
                        child: ListView(
                          ...
                          children: [
                            _nutrientCard('Calories', caloriesProgress, Colors.orange,
                                '${todayCalories.toStringAsFixed(0)}\n${dailyCalorieGoal.toInt()} kcal'),
                            _nutrientCard('Carbs', carbsProgress, _blue,
                                '${todayCarbs.toStringAsFixed(1)}\n${dailyCarbGoal.toInt()}g'),
                            _nutrientCard('Protein', proteinProgress, _purp,
                                '${todayProtein.toStringAsFixed(1)}\n${dailyProteinGoal.toInt()}g'),
                            _nutrientCard('Fat', fatProgress, _amber,
                      ...
                      _wCard(Column(children: [
                        Row(children: [
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            const Text('Weekly Scan Goal',
                                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: _grey1)),
                            const SizedBox(height: 4),
                            Text('${(weeklyGoalProgress * 100).toInt()}% completed',
                                style: const TextStyle(fontSize: 11, color: _grey3)),
                          ])),
                          Text('${(weeklyGoalProgress * 100).toInt()}%',
                              style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: _green)),
                        ]),
                        const SizedBox(height: 10),
                        Stack(children: [
                          Container(height: 8, decoration: BoxDecoration(
                              color: _grey4, borderRadius: BorderRadius.circular(4))),
                      ...
                      const SizedBox(height: 10),
                      recentScans.isEmpty
                          ? _wCard(const Column(children: [
                              Icon(Icons.qr_code_scanner_rounded, size: 36, color: _grey4),
                              SizedBox(height: 8),
                              Text('No food scanned yet', style: TextStyle(color: _grey3, fontSize: 13)),
                            ]))
                          : _wCard(Column(children: recentScans.asMap().entries.map((e) =>
                              Column(children: [
                                _scanRow(e.value),
                                if (e.key < recentScans.length - 1) Divider(height: 1, color: _grey4),
                              ])).toList())),
                      const SizedBox(height: 20),
                      ...
                      const SizedBox(height: 4),
                      Text('Based on your $diabetesType diabetes profile · portions personalized for you',
                          style: const TextStyle(fontSize: 11, color: _grey3, fontStyle: FontStyle.italic)),
                      const SizedBox(height: 10),
                      _buildDoDontCards(),
                      const SizedBox(height: 8),
                    ]),
      ...
      bottomNavigationBar: StreamBuilder<QuerySnapshot>(
        stream: user == null ? const Stream.empty() :
            FirebaseFirestore.instance
                .collection('users').doc(user!.uid).collection('feedback')
                .where('reply', isNotEqualTo: '').snapshots(),
          ...
          return Container(
            ...
            child: BottomNavigationBar(
              ...
              items: [
                ...
                const BottomNavigationBarItem(icon: Icon(Icons.qr_code_scanner_rounded), label: 'Scanner'),
                const BottomNavigationBarItem(icon: Icon(Icons.edit_note_rounded), label: 'Meal Log'),
                BottomNavigationBarItem(
                    icon: _userBadgeIcon(Icons.feedback_rounded, unreadReplies, _currentIndex == 3),
                    label: 'Feedback'),
              ],
            ),
          );
            ...
            Row(children: [
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: ClipOval(child: profilePictureUrl != null && profilePictureUrl!.isNotEmpty
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.monitor_heart, size: 11, color: Colors.white70),
  ...
  Widget _buildGoalsInfoStrip() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(isPersonalized ? Icons.auto_awesome_rounded : Icons.info_outline_rounded,
              size: 13, color: _green),
          const SizedBox(width: 5),
          Text(
            isPersonalized
                ? '$diabetesType Diabetes — Personalized Daily Goals (BMR+TDEE)'
                : '$diabetesType Diabetes — Daily Limits',
        ...
        if (isPersonalized) ...[
          const SizedBox(height: 4),
          Text('Calculated from your height, weight & activity level',
              style: TextStyle(fontSize: 9, color: _green.withOpacity(0.7), fontStyle: FontStyle.italic)),
        ],
      ]),
    );
  }
  ...
  Widget _goalChip(String label) => Container(
        ...
        child: Text(label, style: const TextStyle(fontSize: 10, color: _green, fontWeight: FontWeight.w600)),
      );

  // ── Nutrient circle card ──────────────────────────────────────────────────
  Widget _nutrientCard(String label, double progress, Color color, String detail) {
    ...
    final c = isExceeded ? Colors.red : isCaution ? Colors.orange : color;
    return Container(
      width: 110,
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isExceeded ? Colors.red.withOpacity(0.5)
              : isCaution ? Colors.orange.withOpacity(0.5) : _grey4,
          width: isExceeded || isCaution ? 1.5 : 1,
        ),
        boxShadow: [BoxShadow(color: c.withOpacity(0.1), blurRadius: 8, offset: const Offset(0, 3))],
      ),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Stack(alignment: Alignment.center, children: [
          SizedBox(width: 62, height: 62,
              child: CircularProgressIndicator(
                value: dp, strokeWidth: 6,
          ...
          isExceeded
              ...
              : isCaution
                  ...
                  : Text('${(progress * 100).toInt()}%',
                      style: TextStyle(fontWeight: FontWeight.w800, fontSize: 12, color: c)),
        ]),
        const SizedBox(height: 6),
  ...
  Widget _buildStatusBanner() {
    ...
    if (exceeded.isEmpty && caution.isEmpty) {
      return Container(
        ...
        decoration: BoxDecoration(
          color: _greenPal, borderRadius: BorderRadius.circular(12),
          border: Border.all(color: _green.withOpacity(0.25)),
        ),
        child: Row(children: [
          const Icon(Icons.check_circle_rounded, color: _green, size: 16),
          const SizedBox(width: 8),
          const Expanded(child: Text('✅ All nutrients within your daily limits today!',
    ...
    return Column(children: [
      if (exceeded.isNotEmpty) ...[
        ...
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
            color: _redPal, borderRadius: BorderRadius.circular(12),
            border: Border.all(color: _red.withOpacity(0.3)),
          ),
          child: Row(children: [
            const Icon(Icons.dangerous_outlined, color: _red, size: 16),
            const SizedBox(width: 8),
            Expanded(child: Text('🚨 Limit exceeded: ${exceeded.join(', ')}',
      ...
      if (caution.isNotEmpty) ...[
        ...
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
            color: _amberPal, borderRadius: BorderRadius.circular(12),
            border: Border.all(color: _amber.withOpacity(0.3)),
          ),
          child: Row(children: [
            const Icon(Icons.warning_amber_rounded, color: _amber, size: 16),
            const SizedBox(width: 8),
            Expanded(child: Text('⚠️ Approaching limit: ${caution.join(', ')}',
                style: const TextStyle(fontSize: 12, color: _amber, fontWeight: FontWeight.w600))),
          ]),
        ),
      ],
    ]);
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          const Expanded(child: Text('Log Your Meal',
    ...
    return Padding(
      ...
      child: Row(children: [
        Container(width: 38, height: 38,
            decoration: BoxDecoration(color: _tealPal, borderRadius: BorderRadius.circular(10)),
            child: const Icon(Icons.qr_code_scanner_rounded, color: _teal, size: 18)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: _grey1),
              maxLines: 1, overflow: TextOverflow.ellipsis),
          if (dateStr.isNotEmpty) ...[
            const SizedBox(height: 2),
            Text('$dateStr  •  $timeStr', style: const TextStyle(fontSize: 10, color: _grey3)),
          ],
        ])),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
          decoration: BoxDecoration(color: _tealPal, borderRadius: BorderRadius.circular(20)),
          child: const Text('Scanned', style: TextStyle(fontSize: 10, color: _teal, fontWeight: FontWeight.w700)),
        ),
      ]),
    );
  }
  ...
  Widget _buildProfileStatsRow() {
    ...
    return Wrap(spacing: 6, runSpacing: 4, children: [
      ...
      if (userWeight != null)
        _headerPill(Icons.monitor_weight, '${userWeight!.toStringAsFixed(0)} kg'),
      if (activityLevel != null && actCfg != null)
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(actCfg['icon'] as IconData, size: 10, color: Colors.white),
              const SizedBox(width: 3),
              Text(activityLevel!, style: const TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.w600)),
              const SizedBox(width: 2),
              const Icon(Icons.info_outline, size: 9, color: Colors.white60),
            ]),
          ),
        ),
    ]);
  ...
  Widget _buildDoDontCards() {
    if (diabetesType == 'Not set' ||
      ...
      return _wCard(Center(child: Padding(
      ...
      child: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          return ListView(scrollDirection: Axis.horizontal, children: [
            GestureDetector(
              onTap: () => _openDoDontScreen('✅ DO — Recommended Foods',
                  _buildFoodCardsFromDocs(doFoods), doFoods.length),
              child: _doDontChip('✅ DO', '${doFoods.length} foods', _green, _greenPal),
            ),
            const SizedBox(width: 12),
            GestureDetector(
              onTap: () => _openDoDontScreen("🚫 DON'T — Foods to Avoid",
                  _buildFoodCardsFromDocs(dontFoods), dontFoods.length),
              child: _doDontChip("🚫 DON'T", '${dontFoods.length} foods', _red, _redPal),
            ),
          ]);
        },
      ),
  ...
  Widget _doDontChip(String title, String sub, Color color, Color pal) => Container(
        width: 155,
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: pal,
  ...
  Widget _sLabel(String text) => Text(text,
      style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _grey1));
  ...
  Widget _buildFoodCard(String imageUrl, String foodName,
      Map<String, dynamic> data, PersonalizedNutrition pn) {
            ...
            child: imageUrl.isNotEmpty
                ? Image.network(imageUrl, width: 64, height: 64, fit: BoxFit.cover,
                    loadingBuilder: (_, child, p) => p == null ? child
                        : Container(width: 64, height: 64, color: _grey5,
                            child: const Center(child: CircularProgressIndicator(strokeWidth: 2, color: _green))),
                    errorBuilder: (_, __, ___) => Container(
                        width: 64, height: 64, color: _grey5,
                        child: const Icon(Icons.fastfood_rounded, color: _grey3, size: 28)))
                : Container(
                    width: 64, height: 64,
                    decoration: BoxDecoration(color: _grey5, borderRadius: BorderRadius.circular(10)),
                    child: const Icon(Icons.fastfood_rounded, color: _grey3, size: 28)),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: isPers ? _teal.withOpacity(0.3) : _grey4),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(isPers ? Icons.auto_awesome_rounded : Icons.scale_rounded,
                    size: 10, color: isPers ? _teal : _grey3),
                const SizedBox(width: 4),
                Text(
            ...
            Wrap(spacing: 5, runSpacing: 3, children: [
              _nutBadge('Cal',     '${pn.calories.toStringAsFixed(0)} kcal'),
            ...
            if (isPers) ...[
              ...
              Text('✨ Personalized for your profile',
                  style: TextStyle(fontSize: 9, color: _teal.withOpacity(0.8),
