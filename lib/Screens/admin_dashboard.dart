...
class _AdminDashboardState extends State<AdminDashboard> {
                  ...
                  return StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, fSnap) {
                      ...
                      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          Expanded(child: _bannerChip(Icons.people_alt_rounded,     '$uC',     'Users',        Colors.white, Colors.white.withOpacity(0.18))),
                          const SizedBox(width: 8),
                          Expanded(child: _bannerChip(Icons.edit_note_rounded,      '$lC',     'Food Logs',    Colors.white, Colors.white.withOpacity(0.18))),
                          const SizedBox(width: 8),
                          Expanded(child: _bannerChip(Icons.restaurant_menu_rounded,'$fC',     'Food Rules',   Colors.white, Colors.white.withOpacity(0.18))),
                          const SizedBox(width: 8),
                          Expanded(child: _bannerChip(Icons.trending_up_rounded,    '$active', 'Active Today', Colors.white, Colors.white.withOpacity(0.18))),
                        ]),
                      ]);
                    },
                  );
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                ...
                StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snap) {
                    ...
                    for (final u in snap.data!.docs) {
                      final dt = (u.data() as Map<String, dynamic>)['diabetesType'] ?? '';
                      if (dt == 'Mild') mild++;
                      else if (dt == 'Severe') severe++;
                      else other++;
                    }
                    ...
                    return _card(Column(children: [
                      ...
                      // 3 stat tiles
                      Row(children: [
                        Expanded(child: _typeTile('Mild',    mild,   total, _green,  _greenPal)),
                        const SizedBox(width: 8),
                        Expanded(child: _typeTile('Severe',  severe, total, _red,    _redPal)),
                        if (other > 0) ...[
                          const SizedBox(width: 8),
                          Expanded(child: _typeTile('Other', other,  total, _grey3,  _grey5)),
                        ],
                      ]),
                    ]));
                  },
                ...
                StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snap) {
                    ...
                    for (final f in snap.data!.docs) {
                      final cat = (f.data() as Map<String, dynamic>)['category'] ?? '';
                      if (cat == 'Do') doC++;
                      else if (cat == "Don't") dontC++;
                    }
                    ...
                    return _card(Row(children: [
                      ...
                      Expanded(child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('$total Total Foods',
                              style: const TextStyle(
                                  fontSize: 16,
                                  fontWeight: FontWeight.w800,
                                  color: _grey1)),
                          const SizedBox(height: 10),
                          _foodStatRow(Icons.check_circle_rounded, _green,  _greenPal, 'Recommended', doC),
                          const SizedBox(height: 6),
                          _foodStatRow(Icons.cancel_rounded,        _red,    _redPal,   'To Avoid',    dontC),
                        ],
                      )),
                    ]));
                  },
                ...
                _activityLoading
                    ...
                    : _activityItems.isEmpty
                        ...
                        : _card(Column(children: [
                            ...
                            Builder(builder: (_) {
                              final logs = _activityItems
                                  .where((item) => item['type'] == 'log')
                                  .toList();
                              if (logs.isEmpty)
                              ...
                              return Column(children: [
                                ...
                                _legendDot(_amber, Icons.restaurant_rounded, 'Meal Log'),
                                const SizedBox(height: 12),
                                ...logs.asMap().entries.map((e) {
                                  ...
                                  final time = ts != null ? _ago(ts) : '—';
                                  final mt   = item['mealType'] as String? ?? '';
                                  final badge = mt.isNotEmpty ? mt : 'Food Log';
                                  return _timelineRow(
                                      Icons.restaurant_rounded,
                                      _amber, user, food, badge, time,
                ...
                _card(Column(children: [
                  ...
                  _healthRow2(_storageIcon,                    _storageColor,  'Storage',     _storageStatus),
                  Divider(height: 12, color: _grey4),
                  _healthRow2(Icons.people_rounded,            _purp,          'Users',
                      _healthLoading ? '...' : '$_healthUserCount registered'),
                  Divider(height: 12, color: _grey4),
                  _healthRow2(Icons.restaurant_menu_rounded,   _amber,         'Food Rules',
                      _healthLoading ? '...' : '$_healthFoodCount items'),
                  Divider(height: 12, color: _grey4),
                  _healthRow2(Icons.qr_code_scanner_rounded,   _teal,          'Last Scan',
  ...
  Widget _bannerChip(IconData icon, String value, String label,
      Color fg, Color bg) =>
      Container(
  ...
  Widget _typeTile(String label, int count, int total,
  ...
  Widget _foodStatRow(IconData icon, Color color, Color pal,
  ...
  Widget _timelineRow(IconData icon, Color color, String user,
      String food, String badge, String time,
  ...
  Widget _healthRow2(IconData icon, Color color, String label,
      String status) =>
      Row(children: [
        Container(
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                isExpanded: true,
                style:
                    const TextStyle(color: _grey1, fontSize: 13),
                icon: const Icon(
                ...
                onChanged: (v) =>
                    setState(() => _selectedUserDiabetesType = v!),
              ),
            ),
          ),
        ]),
          ...
          builder: (context, snap) {
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                final dtC   = dt == 'Mild' ? _green : dt == 'Severe' ? _red : _grey3;
                final dtPal = dt == 'Mild' ? _greenPal : dt == 'Severe' ? _redPal : _grey5;
  ...
  Widget _foodsTab() {
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 4),
        const Text("Foods appear in users' Do / Don't screen",
        ...
        _card(Column(children: [
          ...
          const SizedBox(height: 14),
          Row(children: [
            Expanded(child: _dropSection('Category',
              DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  value: category, isExpanded: true,
                  style: const TextStyle(color: _grey1, fontSize: 13),
                  icon: const Icon(Icons.keyboard_arrow_down_rounded, color: _grey3, size: 18),
                  items: const [
                    DropdownMenuItem(value: 'Do',     child: Text('✅ Recommended')),
                    DropdownMenuItem(value: "Don't",  child: Text('❌ Avoid')),
                  ],
                  onChanged: _isUploading ? null : (v) => setState(() => category = v!),
                ),
              ))),
            const SizedBox(width: 10),
            Expanded(child: _dropSection('Diabetes Type',
              DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  value: _selectedDiabetesType, isExpanded: true,
                  style: const TextStyle(color: _grey1, fontSize: 13),
                  icon: const Icon(Icons.keyboard_arrow_down_rounded, color: _grey3, size: 18),
                  items: const [
                  ...
                  onChanged: _isUploading ? null : (v) => setState(() => _selectedDiabetesType = v!),
                ),
              ))),
          ]),
        ])),
  ...
  Widget _sRow(IconData icon, Color color, String label,
      String value) =>
      Padding(
  ...
  Widget _miniStat(
          String label, String value, Color color, Color pal) =>
      Container(
        padding: const EdgeInsets.symmetric(
  ...
  Widget _typeRow(
          String type, int count, int total, Color color) =>
      Padding(
  ...
  Widget _activityRow(IconData icon, String user,
          String action, String time, Color color) =>
      Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(20),
                    border:
                        Border.all(color: dtC.withOpacity(0.3)),
                  ),
                  child: Text(dt,
                      style: TextStyle(
                          fontSize: 11,
          ...
          Container(
            ...
            child: Column(children: [
              ...
              _detailRow(Icons.height_rounded,         _blue,  'Height',        '$height cm'),
              Divider(height: 1, color: _grey4),
              _detailRow(Icons.monitor_weight_rounded, _teal,  'Weight',        '$weight kg'),
              Divider(height: 1, color: _grey4),
              _detailRow(Icons.directions_run_rounded, _amber, 'Activity Level', actLevel),
            ]),
          ),
          const SizedBox(height: 20),
  ...
  Widget _detailRow(IconData icon, Color color,
  ...
  PopupMenuItem<String> _popItem(
    String value, String label, IconData icon, Color color, Color pal,
  ...
  Widget _ff(IconData icon, String hint, TextEditingController ctrl,
      {bool isNum = false}) =>
      TextField(
        ...
        decoration: InputDecoration(
          prefixIcon: Icon(icon,
              color: _isUploading ? _grey3 : _green, size: 18),
          hintText: hint,
          hintStyle:
              const TextStyle(color: _grey3, fontSize: 12),
          filled: true,
  ...
  Widget _dropSection(String label, Widget dropdown) =>
  ...
  Widget _foodFieldFullWidth(IconData icon, String hint,
