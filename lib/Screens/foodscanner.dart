...
class _FoodScannerScreenState extends State<FoodScannerScreen>
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (user == null) { debugPrint('⚠️ No user logged in'); return; }
      ...
      debugPrint('Profile → DT:$_userDiabetesType H:$_userHeight W:$_userWeight A:$_userActivityLevel');
    ...
    try {
      ...
      final response = await http.post(url,
          ...
          body: jsonEncode({
            ...
            'generationConfig': {
              'maxOutputTokens': 100, 'temperature': 0.05, 'topP': 0.9, 'topK': 5,
            },
      ...
      if (response.statusCode != 200) {
        try {
          ...
          _showError('AI error: ${err['error']?['message'] ?? 'API error'}\n\nCheck your API key.');
        } catch (_) { _showError('AI service error ${response.statusCode}.'); }
        return null;
      }
  ...
  String? _parseFoodName(String raw) {
    ...
    for (final p in ['no food','not food','cannot identify','unable to identify',
  ...
  void _matchFoodLocally(String foodName) {
    ...
    for (final doc in _allFoodDocs) {
      ...
      if (score > bestScore) { bestScore = score; best = doc; bestCat = cat; }
    }
    ...
    if (best != null && bestScore >= 15) {
      // ── ALWAYS use admin's category field (Do / Don't) ──────────────
      // This is what admin set — Safe/Avoid is determined by admin, not carbs
      category = _normCat(bestCat);

      // ── Compute personalized nutrition via BMR + TDEE ────────────────
      ...
      debugPrint('Personalized → ${pn.portionLabel} | '
          'Cal:${pn.calories} Carbs:${pn.carbs} Prot:${pn.protein} Fat:${pn.fat}');
      debugPrint('BMR:${pn.bmrValue.toStringAsFixed(0)} '
          'TDEE:${pn.tdeeValue.toStringAsFixed(0)} '
          'Daily:${pn.dailyCalTarget.toStringAsFixed(0)} '
          'Meal:${pn.mealCalBudget.toStringAsFixed(0)}');
    }
  ...
  int _computeScore(List<String> vs, String name, String nameLow, List<String> kws) {
    int best = 0;
    for (final v in vs) {
      ...
      for (final kw in kws) {
        if (kw == v || kw.contains(v) || v.contains(kw)) { best = _mx(best, 60); break; }
      }
  ...
  List<String> _buildVariants(String food) {
    ...
    const map = <String, List<String>>{
      ...
      'talong':['eggplant','aubergine'],'eggplant':['talong'],
      'kangkong':['water spinach','swamp cabbage'],'water spinach':['kangkong'],
      'kalabasa':['squash','pumpkin'],'squash':['kalabasa','pumpkin'],
      'camote':['sweet potato','kamote','yam'],'kamote':['sweet potato','camote','yam'],
      'sweet potato':['camote','kamote'],'gabi':['taro','taro root'],'taro':['gabi'],
      'pechay':['bok choy','pak choi','chinese cabbage'],'bok choy':['pechay','pak choi'],
      'malunggay':['moringa','drumstick leaves'],'moringa':['malunggay'],
      'okra':['lady finger','ladies finger'],'pipino':['cucumber'],'cucumber':['pipino'],
      'kamatis':['tomato'],'tomato':['kamatis'],'repolyo':['cabbage'],'cabbage':['repolyo'],
      'saging':['banana','plantain'],'banana':['saging'],
      'mangga':['mango'],'mango':['mangga'],'langka':['jackfruit'],'jackfruit':['langka'],
      'pinya':['pineapple'],'pineapple':['pinya'],'pakwan':['watermelon'],'watermelon':['pakwan'],
      'avocado':['abukado'],'abukado':['avocado'],
      'kanin':['white rice','rice','plain rice'],
      'white rice':['rice','kanin','plain rice','steamed rice'],
      'rice':['white rice','kanin','plain rice'],'brown rice':['rice','whole grain rice'],
      'sinangag':['garlic rice','fried rice'],'garlic rice':['sinangag','fried rice'],
      'lugaw':['congee','rice porridge','porridge'],'congee':['lugaw','porridge'],
      'oats':['oatmeal','rolled oats'],'oatmeal':['oats','rolled oats'],
      'manok':['chicken','fried chicken','grilled chicken'],
      'chicken':['manok','fried chicken','grilled chicken'],'fried chicken':['chicken','manok'],
      'baboy':['pork','lechon','liempo'],'pork':['baboy','liempo','lechon'],
      'liempo':['pork belly','pork','baboy'],'lechon':['roast pork','pork','baboy'],
      'baka':['beef'],'beef':['baka'],
      'isda':['fish','tilapia','bangus'],'fish':['isda','tilapia','bangus','galunggong'],
      'bangus':['milkfish','fish','isda'],'milkfish':['bangus','fish'],
      'tilapia':['fish','isda'],'galunggong':['fish','isda','mackerel scad'],
      'sardinas':['sardines','canned fish'],'sardines':['sardinas'],
      'hipon':['shrimp','prawns'],'shrimp':['hipon','prawns'],
      'pusit':['squid'],'squid':['pusit'],
      'itlog':['egg','eggs','boiled egg','fried egg'],'egg':['itlog','eggs','boiled egg'],
      'eggs':['itlog','egg'],'tokwa':['tofu','bean curd'],'tofu':['tokwa','bean curd'],
      'adobo':['chicken adobo','pork adobo','adobong manok'],
      'sinigang':['pork sinigang','sinigang na baboy'],
      'tinola':['chicken tinola','tinolang manok'],
      'pinakbet':['pakbet','mixed vegetables'],'pakbet':['pinakbet'],
      'pancit':['noodles','bihon','canton'],'noodles':['pancit','bihon','canton'],
      'pandesal':['bread','pan de sal'],'bread':['pandesal','tinapay'],
      'mais':['corn'],'corn':['mais'],
    };
    if (map.containsKey(food)) s.addAll(map[food]!);
    for (final e in map.entries) {
      if (food != e.key && (food.contains(e.key) || e.key.contains(food))) {
        s.addAll(e.value); s.add(e.key);
      }
    }
    return s.toList();
  }
  ...
  Future<void> _saveHistory() async {
    try {
      ...
      await FirebaseFirestore.instance
          .collection('users').doc(user.uid).collection('scanned_foods').add({
        'food':      _detectedFood,
        'category':  _category,
        ...
        if (_cameraPermDenied)
          ...
          const Positioned.fill(child: Center(child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('Starting camera...', style: TextStyle(color: Colors.white, fontSize: 15)),
            ],
          ))),
        ...
        Positioned(top: 0, left: 0, right: 0, child: SafeArea(
          child: Padding(
            ...
            child: Row(children: [
              ...
              _topBtn(_flashOn ? Icons.flash_on : Icons.flash_off, _toggleFlash, active: _flashOn),
            ]),
          ),
        )),
        ...
        if (!_showResult && !_isLoading && _isCameraReady)
          Positioned(bottom: 48, left: 0, right: 0, child: Column(children: [
            Container(
              ...
              child: const Text('Point at food and tap Scan',
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(Icons.center_focus_strong, color: Colors.white, size: 36),
              ),
            ),
          ])),
      ]),
  ...
  Widget _buildPermissionDenied() => Positioned.fill(child: Container(
  ...
  Widget _buildScanFrame() {
    ...
    return Stack(children: [
      Positioned(top: 0, left: 0, right: 0, height: frameTop, child: Container(color: Colors.black54)),
      Positioned(top: frameTop, left: 0, width: frameLeft, height: frameSize, child: Container(color: Colors.black54)),
      Positioned(top: frameTop, right: 0, width: frameLeft, height: frameSize, child: Container(color: Colors.black54)),
      Positioned(top: frameTop + frameSize, left: 0, right: 0, bottom: 0, child: Container(color: Colors.black54)),
      Positioned(top: frameTop,                  left: frameLeft,  child: _corner(true,  true)),
      Positioned(top: frameTop,                  right: frameLeft, child: _corner(true,  false)),
      Positioned(top: frameTop + frameSize - 32, left: frameLeft,  child: _corner(false, true)),
      Positioned(top: frameTop + frameSize - 32, right: frameLeft, child: _corner(false, false)),
      if (!_isLoading && !_showResult)
        _ScanLine(frameTop: frameTop, frameSize: frameSize, frameLeft: frameLeft),
    ]);
  }

  Widget _corner(bool top, bool left) => SizedBox(width: 32, height: 32,
      child: CustomPaint(painter: _CornerPainter(
          color: const Color(0xFF4CAF50), strokeWidth: 4, top: top, left: left)));
  ...
  Widget _buildResultPanel() {
    ...
    String   catLabel;
    switch (_category) {
      case 'do':   catColor = Colors.green;  catIcon = Icons.check_circle;  catLabel = 'Safe to Eat';    break;
      case 'dont': catColor = Colors.red;    catIcon = Icons.cancel;        catLabel = 'Avoid This Food'; break;
      default:     catColor = Colors.orange; catIcon = Icons.help_outline;  catLabel = 'Not in Database';
    }
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_capturedImagePath != null)
          Center(child: Container(
            ...
            child: ClipRRect(
              ...
              child: Image.file(File(_capturedImagePath!), width: 140, height: 140, fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) => Container(width: 140, height: 140,
                      color: Colors.grey[800],
                      child: const Icon(Icons.broken_image, color: Colors.white54, size: 40))),
            ),
          )),

        // Food name + safe/avoid badge
        Row(children: [
          Expanded(child: Text(_cap(_detectedFood),
        ...
        if (rule != null) ...[
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
                  color: const Color(0xFF4CAF50).withOpacity(0.6), width: 1.5),
            ),
            child: Row(children: [
              Container(
              ...
              Expanded(child: Column(
                ...
                children: [
                  Text(
                    isPers ? 'YOUR PERSONALIZED PORTION' : 'RECOMMENDED PORTION',
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Text(
                  isPers ? 'NUTRITION FOR YOUR PORTION' : 'NUTRITION INFO (per 100g)',
              ...
              if (isPers) ...[
                ...
                Text(
                  ...
                  style: const TextStyle(
                      color: Colors.white24, fontSize: 9, fontStyle: FontStyle.italic),
                ),
              ],
            ]),
          ),
        ...
        Container(
          ...
          child: Text(
            ...
            style: TextStyle(
                color: catColor, fontWeight: FontWeight.bold, fontSize: 13, height: 1.4),
            textAlign: TextAlign.center,
          ),
        ),
        const SizedBox(height: 14),
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: const Text('Scan Again', style: TextStyle(color: Colors.white, fontSize: 16)),
            onPressed: () {
              if (_capturedImagePath != null) {
                try { File(_capturedImagePath!).delete(); } catch (e) { debugPrint('Delete: $e'); }
              }
              setState(() {
                _showResult            = false;
                _detectedFood          = '';
  ...
  // ── Convert grams → human-readable portion (slices, cups, pieces, etc.) ──
  // Converts grams to a natural human-readable portion
  // e.g. "½ small apple", "1 medium banana", "2 small pieces ampalaya"
  String _gramsToHumanPortion(double grams, String foodName) {
    ...
    if (lower.contains('apple'))
      return natural(120, 'apple', grams < 80 ? 'small' : grams < 160 ? 'medium' : 'large');
    if (lower.contains('banana') || lower.contains('saging'))
      return natural(90, 'banana', grams < 70 ? 'small' : grams < 120 ? 'medium' : 'large');
    if (lower.contains('mango') || lower.contains('mangga'))
      return natural(200, 'mango', grams < 150 ? 'small' : grams < 280 ? 'medium' : 'large');
    ...
    if (lower.contains('ampalaya') || lower.contains('bitter'))
      return natural(60, 'piece', grams < 50 ? 'small' : grams < 100 ? 'medium' : 'large');
    if (lower.contains('talong') || lower.contains('eggplant'))
      return natural(80, 'piece', grams < 60 ? 'small' : grams < 120 ? 'medium' : 'large');
    ...
    if (lower.contains('tomato') || lower.contains('kamatis'))
      return natural(80, 'tomato', grams < 60 ? 'small' : grams < 120 ? 'medium' : 'large');
    if (lower.contains('okra'))
      return natural(15, 'piece okra', 'small');
    if (lower.contains('sitaw') || lower.contains('string bean'))
      return natural(20, 'stalk', 'long');
    if (lower.contains('kangkong') || lower.contains('pechay')
        || lower.contains('cabbage') || lower.contains('repolyo')
        || lower.contains('malunggay'))
      return natural(50, 'handful', grams < 50 ? 'small' : 'medium');

    // ── ROOT CROPS ───────────────────────────────────────────────────
    if (lower.contains('camote') || lower.contains('kamote')
        || lower.contains('sweet potato'))
      return natural(130, 'camote', grams < 100 ? 'small' : grams < 180 ? 'medium' : 'large');
    if (lower.contains('gabi') || lower.contains('taro'))
      return natural(80, 'piece', grams < 60 ? 'small' : 'medium');
    if (lower.contains('potato'))
      return natural(150, 'potato', grams < 100 ? 'small' : grams < 200 ? 'medium' : 'large');
    ...
    if (lower.contains('shrimp') || lower.contains('hipon')
    ...
    if (lower.contains('egg') || lower.contains('itlog'))
      return natural(55, 'egg', grams < 45 ? 'small' : grams < 65 ? 'medium' : 'large');
    ...
    if (lower.contains('pandesal'))
      return natural(40, 'pandesal', grams < 35 ? 'small' : grams < 55 ? 'medium' : 'large');
    if (lower.contains('bread') || lower.contains('toast'))
      return natural(30, 'slice bread', grams < 30 ? 'thin' : 'regular');
    if (lower.contains('bibingka') || lower.contains('puto')
        || lower.contains('kutsinta') || lower.contains('kakanin'))
      return natural(60, 'piece', grams < 50 ? 'small' : 'medium');
    if (lower.contains('cake') || lower.contains('pie')
        || lower.contains('pizza') || lower.contains('lasagna'))
      return sliced(90, lower.contains('pizza') ? 'pizza' : 'cake', grams < 70 ? 'thin' : 'medium');

    // ── RICE & GRAINS ─────────────────────────────────────────────────
    if (lower.contains('rice') || lower.contains('kanin')
        || lower.contains('sinangag') || lower.contains('garlic rice'))
      return natural(120, 'scoop of rice', grams < 100 ? 'small' : grams < 180 ? 'medium' : 'large');
    ...
    // ── NOODLES ───────────────────────────────────────────────────────
    if (lower.contains('pancit') || lower.contains('noodle')
        || lower.contains('bihon') || lower.contains('canton'))
      return natural(150, 'serving of pancit', grams < 120 ? 'small' : 'medium');
    ...
    // ── DEFAULT ───────────────────────────────────────────────────────
    final size = grams < 80 ? 'small' : grams < 160 ? 'medium' : 'large';
...
class _ScanLine extends StatefulWidget {
  ...
  const _ScanLine({required this.frameTop, required this.frameSize, required this.frameLeft});
  @override
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        ...
        return Positioned(
          ...
          right: MediaQuery.of(context).size.width - widget.frameLeft - widget.frameSize + 4,
          child: Container(
            height: 2.5,
            decoration: BoxDecoration(
...
class _CornerPainter extends CustomPainter {
  ...
  final bool   top;
  final bool   left;
  const _CornerPainter({required this.color, required this.strokeWidth, required this.top, required this.left});
  @override
  void paint(Canvas canvas, Size size) {
    ...
    if (top && left)       { path.moveTo(0, h); path.lineTo(0, 0); path.lineTo(w, 0); }
    else if (top && !left) { path.moveTo(0, 0); path.lineTo(w, 0); path.lineTo(w, h); }
    else if (!top && left) { path.moveTo(0, 0); path.lineTo(0, h); path.lineTo(w, h); }
    else                   { path.moveTo(0, h); path.lineTo(w, h); path.lineTo(w, 0); }
    canvas.drawPath(path, paint);
  }
  @override
  bool shouldRepaint(_CornerPainter o) => o.color != color || o.strokeWidth != strokeWidth;
}